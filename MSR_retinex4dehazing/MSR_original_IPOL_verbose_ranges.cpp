...
void print_min_max_3ch_indep(double *R, double *G, double *B, size_t image_size, char text[20])
{
...
int main(int argc,  char **argv)
{
    ...
    print_min_max_3ch_indep(R, G, B, image_size, "RGB_pre_input_inversion");
    ...
    print_min_max_3ch_indep(R, G, B, image_size, "RGB_post_input_inversion");
#endif
    ...
    /*  aitor */
    //printf("Applying the Simplest Color Balance BEFORE retinex with saturation values (%.1f, %.1f)\n", s1, s2);

    //print_min_max_3ch_indep(R, G, B, image_size, "RGB_pre_SCB");
    //simplest_color_balance(R, R, image_size, s1, s2);
    //simplest_color_balance(G, G, image_size, s1, s2);
    //simplest_color_balance(B, B, image_size, s1, s2);
    //print_min_max_3ch_indep(R, G, B, image_size, "RGB_post_SCB");
    /*  \aitor */

    /* MSR on RGB channels*/
    /* inputs are double, outputs are double */
    printf("Applying the MSRetinex to the 3ch independently \n");
    print_min_max_3ch_indep(R, G, B, image_size, "RGB_pre_MSR");
    MSRetinex( Rout,  R,  scale, nscales, w, nx,ny);
    MSRetinex( Gout,  G,  scale, nscales, w, nx,ny);
    MSRetinex( Bout,  B,  scale, nscales, w, nx,ny);
    print_min_max_3ch_indep(Rout, Gout, Bout, image_size, "RGB_post_MSR");
...
//#if 0
    ...
    else  /* Simplest color balance with s1% and s2% of saturation*/
    {
        ...
        printf("Applying the Simplest Color Balance, NOT the Gain/Offset approach\n");
